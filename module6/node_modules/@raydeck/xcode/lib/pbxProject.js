...
pbxProject.prototype.addToFrameworkSearchPaths = function(file) {
  ...
  for (config in configurations) {
    ...
    if (
      ...
      buildSettings["FRAMEWORK_SEARCH_PATHS"] = [INHERITED];
    } else if (typeof buildSettings["FRAMEWORK_SEARCH_PATHS"] === "string") {
      const fsp = buildSettings["FRAMEWORK_SEARCH_PATHS"];
      buildSettings["FRAMEWORK_SEARCH_PATHS"] = [INHERITED, fsp];
    }
    try {
...
pbxProject.prototype.removeFromSearchPathsByRegex = function(
  regex,
  configName
) {
  var configurations = nonComments(this.pbxXCBuildConfigurationSection());
  for (config in configurations) {
    buildSettings = configurations[config].buildSettings;
    if (unquote(buildSettings["PRODUCT_NAME"]) != this.productName) continue;
    if (buildSettings[configName]) {
      var unmatches = buildSettings[configName].filter(p => {
        //Unwrap string
        if (p.startsWith('"')) p = p.substr(1);
        return !p.match(regex);
      });
      buildSettings[configName] = unmatches;
    }
  }
};
pbxProject.prototype.removeFromHeaderSearchPathsByRegex = function(regex) {
  return this.removeFromSearchPathsByRegex(regex, "HEADER_SEARCH_PATHS");
};
pbxProject.prototype.removeFromFrameworkSearchPathsByRegex = function(regex) {
  return this.removeFromSearchPathsByRegex(regex, "FRAMEWORK_SEARCH_PATHS");
};
...
//Pragma - RHD Added functionality
pbxProject.prototype.addSourceFileNew = function(placeholder) {
  console.log("Starting with ", placeholder);
  const fp = this.getFirstProject();
  let file = this.addFile(placeholder, null, fp);
  if (!file) {
    console.log("Looks like the file is already here - aborting", placeholder);
    return;
  }
  file.uuid = this.generateUuid();
  const nts = this.pbxNativeTargetSection();
  for (var key in nts) {
    if (key.endsWith("_comment")) continue;
    const target = this.pbxTargetByName(nts[key].name);
    file.target = key;
    this.addToPbxBuildFileSection(file); // PBXBuildFile
    this.addToPbxSourcesBuildPhase(file);
  }
